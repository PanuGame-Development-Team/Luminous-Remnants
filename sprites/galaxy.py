...
class Galaxy(pygame.sprite.Sprite):
    def __init__(self,name,label,center,sidls,stars,labelfont,screensize,*groups):
        ...
        for star in self.stars.sprites():
            self.left = min(self.left,star.pos[0] - STAR.RADIUS)
            self.right = max(self.right,star.pos[0] + STAR.RADIUS)
    ...
    def update(self,screen,speed,mouse,alpha):
        ...
        linecolor = color_adapt(GALAXY.LINE_COLOR,GENERAL.BG_COLOR,alpha,GALAXY.LINE_SHOW_FACTOR,dist)
        fontcolor = color_adapt(GALAXY.LABEL_COLOR,GENERAL.BG_COLOR,alpha,GALAXY.LABEL_SHOW_FACTOR,dist)
        ...
        if not self.label:
            ...
            screen.blit(galnamesurf,centrialize(*self.center,*galnamesurf.get_size(),0,-GALAXY.LABEL_DISPSIZE/2))
            screen.blit(labelsurf,centrialize(*self.center,*labelsurf.get_size(),0,GALAXY.LABEL_DISPSIZE/2))
